...
function MonthReport() {
  ...
  const getReport = async () => {
    ...
    const url = "http://127.0.0.1:8000/api/v1/report/all/month";
  ...
  const csvMetarHeader = [
    ...
    { label: "Prévue", key: "stat_info.expected_m" },
                          ...
                          {reports.map((rpt) => {
                            return (
                              <tr key={rpt.stat_info.stat_id}>
                                ...
                                {rpt.stat_info.taux ? (
                                  <td>{rpt.stat_info.taux.expected_m}</td>
                                ) : (
                                  <td>{rpt.stat_info.expected_m}</td>
                                )}
                                <td>{rpt.rec_num}</td>
                                <td>{rpt.month_prod}%</td>
                                <td>{rpt.s_prodh1}%</td>
                                <td>{rpt.s_prodh2}%</td>
                                <td>{rpt.s_rtdH1}%</td>
                                <td>{rpt.s_rtdH2}%</td>
